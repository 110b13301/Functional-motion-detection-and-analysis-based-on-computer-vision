        ...
        <div class="col-lg-9 col-md-12">
          ...
          <div class="col-lg-12 d-flex">
            ...
            <div style="flex: 1; padding-left: 20px; border-left: 2px solid #ccc; font-size: 1.5rem;">
              ...
              <!-- 左邊影片-->
              <!-- <div style="flex: 2;  font-size: 1.5rem; ">
              <h1>結果</h1>
                <div id="total_record">總分數: </div>
                <div id="knee_score" >膝蓋角度: </div>
                <div id="bottom_score">臀部角度: </div>
...
const total_record = parseInt(localStorage.getItem('fmsScore')) ;
const knee_score = parseFloat(localStorage.getItem('KneeAngle')) ;
const bottom_score = parseFloat(localStorage.getItem('BottomAngle')) ;

document.getElementById('total_record').innerText = `總分數: ${total_record} 分`;
document.getElementById('knee_score').innerText = `膝蓋角度: ${!isNaN(knee_score) ? knee_score.toFixed(2) : '尚未檢測'}`;
document.getElementById('bottom_score').innerText = `臀部角度: ${!isNaN(bottom_score) ? bottom_score.toFixed(2) : '尚未檢測'}`;
function getEvaluation(score) {
      if (score == 3) {
        return "1.軀幹和脛骨或小腿平行<br>2.股骨低於水平線<br>3.膝蓋無內扣<br>4.長桿始終保持頭上方";
      } else if (score == 2) {
        return "1.腳跟有墊高<br>2.軀幹和脛骨或小腿平行<br>3.股骨低於水平線<br>4.膝蓋無內扣<br>5.長桿始終保持頭上方";
      } else if (score == 1) {
        return "1.腳跟墊高了還不能做到起始姿勢，也不能完全下蹲<br>2.出現膝蓋內扣或者長桿過度向前移動等情況";
      } else {
        return "尚未評分或數據錯誤。";
      }
    }
    const evaluationText = total_record && !isNaN(total_record) ? getEvaluation(total_record) : "尚未檢測";
...
document.getElementById('historyBtn').addEventListener('click', () => {
    ...
    fetch('recordfms1.php', {
      ...
      .then(result => {
          alert('資料已成功提交！');
          console.log(result);
          window.location.href = 'historyfms1.php';
      })
      .catch(error => {
  ...
  document.getElementById('submitBtn').addEventListener('click', () => {
    ...
    fetch('recordfms1.php', {
      ...
      .then(result => {
          alert('資料已成功提交！');
          console.log(result);
          window.location.href = 'FMS1.html';
      })
      .catch(error => {
          console.error('Error:', error);
          alert('提交資料失敗，請稍後再試');
