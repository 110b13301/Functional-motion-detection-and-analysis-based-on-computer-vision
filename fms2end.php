        ...
        <ul class="navbar-nav mx-auto">
          ...
          <li class="nav-item dropdown">
            ...
            <ul class="dropdown-menu">
              ...
              <li><a class="dropdown-item" href="blog-single.html">總歷史紀錄紀錄</a></li>
            </ul>
          </li>
          ...
          <div class="row">
            <div class="col-lg-12 d-flex">
              ...
              <div style="flex: 1; padding-left: 20px; border-left: 2px solid #ccc; font-size: 1.5rem; ">
                ...
                <div id="knee_score" >膝蓋角度: </div>
                <div id="bottom_score">臀部角度: </div>
                <h2>評分</h2>
                  3分:腳跟沒有墊高且蹲的下去。<br>
                  2分:腳跟有墊高且蹲的下去。<br>
                  1分:腳跟有墊高且蹲不下去。<br>
              ...
              <h1>結果</h1>
                <div id="left_score">左腳分數: </div>
                <div id="right_score">右腳分數: </div>
                <div id="total_score">總分數: </div>
              </div>
...
const left_score = localStorage.getItem('leftFootScore') ;
const right_score = localStorage.getItem('rightFootScore');
const total_score = localStorage.getItem('totalScore') ;
function getEvaluation(score) {
      if (score == 3) {
        return "1.髖、膝、踝保持一直線<br>2.腰椎無移動<br>3.棍子與欄架保持平行";
      } else if (score == 2) {
        return "1.髖、膝、踝未保持一直線<br>2.腰椎移動<br>3.棍子與欄架未保持平行";
      } else if (score == 1) {
        return "1.未能保持平衡<br>2.腳與欄架發生接觸";
      } else {
        return "尚未評分或數據錯誤。";
      }
    }
    const evaluationText = total_score && !isNaN(total_score) ? getEvaluation(total_score) : "尚未檢測";
    document.getElementById('evaluation').innerHTML = `${evaluationText}`;
// 检查分数是否存在并显示
document.getElementById('left_score').innerText = `左腳分數: ${!isNaN(left_score) ? left_score: '尚未檢測'} 分`;
document.getElementById('right_score').innerText = `右腳分數: ${!isNaN(right_score) ? right_score: '尚未檢測'} 分`;
document.getElementById('total_score').innerText = `總分數: ${!isNaN(total_score) ? total_score: '尚未檢測'} 分`;
...
document.getElementById('historyBtn').addEventListener('click', () => {
    ...
    fetch('recordfms2.php', {
      ...
      .then(result => {
          alert('資料已成功提交！');
          console.log(result);
          window.location.href = 'historyfms2.php';
      })
      .catch(error => {
