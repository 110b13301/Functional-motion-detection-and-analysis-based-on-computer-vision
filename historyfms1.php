...
// 根據選擇的日期查詢資料
$sql = "SELECT * FROM recordfms1 WHERE user_id = ? ";
$params = [$user_id];
	...
	<div class="row">
		<div class="col-lg-12">
			...
			<div id="day" class="post-tags my-5 text-uppercase font-size-12 letter-spacing text-center">
				...
				<br><br>
				<div id="content0"></div>
                    <form method="POST" action="historyfms1.php">
                        ...
                        <tbody>
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($row['knee_score']); ?></td>
                                    <td><?php echo htmlspecialchars($row['bottom_score']); ?></td>
		...
		<div id="month" class="post-tags my-5 text-uppercase font-size-12 letter-spacing text-center">
			...
			<?php
				...
				$selected_date = isset($_POST['selected_date']) ? $_POST['selected_date'] : date('Y-m-d');
				$timestamp = strtotime($selected_date);
				$start_date = date('Y-m-01', $timestamp);  // 當月的第一天
				$end_date = date('Y-m-t', $timestamp);     // 當月的最後一天
				$selected_month = date('Y年n月', $timestamp);
				// 計算當前月份
				$current_month = date('Y年n月');

				// 根據當月日期查詢每日最高的 total_record
				$sql = "SELECT date, MAX(total_record) as max_total_record FROM recordfms1 WHERE user_id = ? AND date BETWEEN ? AND ? GROUP BY date";
				$stmt = $conn->prepare($sql);
				...
				while ($row = $result->fetch_assoc()) {
				    $day = (int)date('j', strtotime($row['date'])); // 取得當日 (1-31)
				    $activity_data[$day] = $row['max_total_record']; // 儲存每一天的最大值
				}
				?>
				<h2 id="month-title"><?php echo $selected_month; ?></h2>
