...
<script>
        ...
        <div class="row">
          <div class="col-lg-12 d-flex">
            ...
            <div style="flex: 1; padding-left: 20px; border-left: 2px solid #ccc; font-size: 1.5rem;">
              ...
              <h2>評分標準</h2>
              3分:上腿腳踝的垂直線應落在大腿中點和髂前上棘之間，而下腿維持在起始姿勢上。<br>
              2分:上腿腳踝的垂直線落在大腿中點和髕骨中點之間，同時下腿維持在起始姿勢上。<br>
              1分:上腿腳踝的垂直線落在髕骨中點以下，同時下腿維持在起始姿勢上。<br>
            </div>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    const R_distance =localStorage.getItem('RightLegAngle') ; // 右抬腿成功角度
    const L_distance = localStorage.getItem('LeftLegAngle'); // 左抬腿成功角度
    ...
    document.getElementById('R_distance').innerText = `右抬腿成功角度:  ${R_distance ? R_distance+ ' 度' : '尚未檢測'}`;
    document.getElementById('L_distance').innerText = `左抬腿成功角度: ${L_distance ? L_distance + ' 度' : '尚未檢測'}`;
    // 提交資料到後端並查看歷史紀錄
    ...
    const data = {
      ...
      .then(result => {
          alert('資料已成功提交！');
          console.log(result);
          window.location.href = 'historyfms5.php'; // 提交後查看歷史紀錄
      })
      .catch(error => {
    ...
    const data = {
      ...
      .then(result => {
          alert('資料已成功提交！');
          console.log(result);
          window.location.href = 'FMS5.html'; // 提交後查看歷史紀錄
      })
      .catch(error => {
          console.error('Error:', error);
      });
