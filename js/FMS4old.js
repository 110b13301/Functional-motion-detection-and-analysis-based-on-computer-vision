...
const dingDongSound = new Audio('pass.mp3');
...
document.getElementById("confirm-button").addEventListener("click", function() {
  window.location.href = 'fms4end.php'; // 跳轉到下一頁
});
...
function onResults(results) {
  ...
  const PIXEL_TO_CM = 0.0264; // 每個像素 0.0264 公分
...
if (currentTask === '兩隻手拇指包住握拳') {
  ...
  if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
    for (let index = 0; index < results.multiHandLandmarks.length; index++) {
      ...
      if (currentTask === '背對螢幕，右手過頭頂在背後，向下伸，然後左手盡量靠近右手') {
        if (results.multiHandLandmarks.length === 2) {
          ...
          const distanceCm = distance * PIXEL_TO_CM * 6.4;
    ...
    if (currentTask === '伸出自己的慣用手，手掌打開面對鏡頭') {
      conditionMet = results.multiHandLandmarks.length === 1 && isHandInBox(results.multiHandLandmarks[0], boxX, boxY, boxWidth, boxHeight); // 檢查是否只有一隻手掌且在紅框內
      if (!(results.multiHandLandmarks && results.multiHandLandmarks.length === 1 && isHandInBox(results.multiHandLandmarks[0], boxX, boxY, boxWidth, boxHeight))) {
        drawTextWithBackground(canvasCtx, '請手在紅框中', canvasElement.width / 2, canvasElement.height / 2, "red", "black");
      }
    } else if (currentTask === '兩隻手拇指包住握拳' && results.multiHandLandmarks.length === 2) {
      // 兩隻手的手掌信息（僅當檢測到兩隻手時）
