...
<script>
  ...
  <div class="main-wrapper ">
          ...
          <div class="row">
            <div class="col-lg-12 d-flex">
              ...
              <div style="flex: 2; padding-right: 20px;">
                <video src="影片位置" width="100%" height="auto" autoplay="true" controls="true" alt="" class="img-fluid"></video>
              </div>
              <div style="flex: 1; padding-left: 20px; border-left: 2px solid #ccc; font-size: 1.5rem; ">
                <h1>結果</h1>
                <div id="total-score">總分數: </div>
                <div id="hand-size" >手掌大小: </div>
                <div id="fists-distance1">右手拳距離: </div>
                <div id="score_R">右手得分: </div>
                <div id="fists-distance2">左手拳距離: </div>
                <div id="score_L">左手得分: </div>
                <h2>評分</h2>
                  3分:雙拳之間的距離應小於一個手掌的長度。<br>
                  2分:雙拳之間的距離小於一個半手掌的長度。<br>
                  1分:雙拳之間的距離大於一個半手掌的長度。<br>
                  0分:在執行動作的過程中感到疼痛。 -->
                                <!-- 左邊影片 -->
              <div style="flex: 1; padding-left: 20px; font-size: 1.5rem;">
              <h1>結果</h1>
                <div id="total-score">總分數: </div>
                <div id="hand-size" >手掌大小: </div>
                <div id="fists-distance1">右手拳距離: </div>
    ...
    <script>
    const handSize = localStorage.getItem('handSize');
    const fistsDistance1 = localStorage.getItem('fistsDistance1');
    const score_R = localStorage.getItem('score_R');
    const fistsDistance2 = localStorage.getItem('fistsDistance2');
    const score_L = localStorage.getItem('score_L');
    const totalScore = score_R && score_L ? Math.min(parseInt(score_R), parseInt(score_L)) : '尚未計算';

    function getEvaluation(score) {
      if (score == 3) {
        return "雙拳之間的距離應小於一個手掌的長度。";
      } else if (score == 2) {
        return "雙拳之間的距離小於一個半手掌的長度。";
      } else if (score == 1) {
        return "雙拳之間的距離大於一個半手掌的長度。";
      } else {
        return "尚未評分或數據錯誤。";
      }
    }
    ...
    document.getElementById('total-score').innerText = `總分數: ${totalScore} 分`;
    document.getElementById('hand-size').innerText = `手掌大小: ${handSize ? handSize + ' cm' : '尚未檢測'}`;
    document.getElementById('fists-distance1').innerText = `右手拳距離: ${fistsDistance1 ? fistsDistance1 + ' cm' : '尚未檢測'}`;
    document.getElementById('score_R').innerText = `右手得分: ${score_R ? score_R + ' 分' : '尚未檢測'}`;
    document.getElementById('fists-distance2').innerText = `左手拳距離: ${fistsDistance2 ? fistsDistance2 + ' cm' : '尚未檢測'}`;
    ...
    const user_id = <?php echo json_encode($_SESSION['user_id']); ?>; // 使用者ID
      ...
      const data = {
          ...
          L_distance: parseFloat(fistsDistance2),
          R_distance: parseFloat(fistsDistance1),
          date: new Date().toISOString().split('T')[0],
          time: new Date().toTimeString().split(' ')[0]
      };

      fetch('recordfms4.php', {
          method: 'POST',
          headers: {
      ...
      .then(result => {
          alert('資料已成功提交！');
          console.log(result);
          window.location.href = 'historyfms4.php'; // 提交後查看歷史紀錄
      })
      .catch(error => {
    ...
    // 提交按鈕事件
      ...
      const data = {
          sport_ID: 'FMS4',
          user_id: user_id,  // 使用 session 中的 user_id
          total_record: Math.min(parseInt(score_R), parseInt(score_L)),
          score_L: parseInt(score_L),
          score_R: parseInt(score_R),
          L_distance: parseFloat(fistsDistance2),
          R_distance: parseFloat(fistsDistance1),
          date: new Date().toISOString().split('T')[0],  // 僅日期部分
          time: new Date().toTimeString().split(' ')[0]   // 僅時間部分
      };
      ...
      .then(result => {
          alert('資料已成功提交！');
          console.log(result);
          window.location.href = 'FMS4.html'; 
      })
      .catch(error => {
          console.error('Error:', error);
      });
